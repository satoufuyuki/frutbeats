    ...
    <>
      ...
      <div className="flex gap-x-4 flex-col md:flex-row gap-y-2">
        ...
        {!playlistData.images.length && (
          <div className="w-[150px] h-[150px] rounded-lg bg-gray-400 flex">
            <FontAwesomeIcon icon={faMusic} className="m-auto" size="2x" />
          </div>
        )}
        <div className="flex flex-col gap-y-2">
          ...
          <button className="btn btn-primary btn-circle text-white" onClick={() => playPlaylist(playlistData.uri)}>
      ...
      <div className="flex flex-col w-full gap-y-2 mt-12 ">
        {playlistData.tracks.items.map((x: any, i: number) => (
          <PlaylistItem key={i} name={x.track?.name ?? "Unknown"} artist={x.track?.artists[0]?.name ?? "Unknown"} image={x.track?.album.images[0]?.url} uri={x.track?.uri} id={x.track?.id} />
        ))}
      </div>
