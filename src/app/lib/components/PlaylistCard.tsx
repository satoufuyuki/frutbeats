...
export function PlaylistCard({ skeleton = false, id, image, title, description }: { id?: string, skeleton?: boolean; image?: string; title?: string; description?: string }) {
    ...
    return <div onClick={() => skeleton ? undefined : router.push(`/playlist/${id}`)} className="cursor-pointer flex flex-col p-4 bg-base-300 hover:bg-base-200 rounded-lg shadow">
        {skeleton && (
            <>
                <div className="w-[194px] h-[194px] bg-gray-400 animate-pulse rounded-md"/>
        ...
        {!skeleton && (
            ...
            <Image src={image!}
                width={194}
                height={194}
                loading={"lazy"} alt="Recommendation Cover"/>
