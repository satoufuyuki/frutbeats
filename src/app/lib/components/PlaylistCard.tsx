...
export function PlaylistCard({ skeleton = false, id, image, title, description }: { id?: string, skeleton?: boolean; image?: string; title?: string; description?: string }) {
    ...
    return <div onClick={() => skeleton ? undefined : router.push(`/playlist/${id}`)}
        className="w-full cursor-pointer flex flex-col p-4 bg-base-300 hover:bg-base-200 rounded-lg shadow">
        {skeleton && (
            <>
        ...
        {!skeleton && (
            ...
            <Image src={image!}
                width={194}
                height={194}
                loading={"lazy"} alt="Recommendation Cover" className="w-full"/>
