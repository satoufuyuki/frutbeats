...
export function SidebarPlaylist() {
  ...
  const fetchPlaylists = async ({ pageParam = 0 }) => {
    const res = await fetch(`https://api.spotify.com/v1/me/playlists?offset=${pageParam}&limit=20`, {
      headers: {
  ...
  return (
    <InfiniteScroll scrollableTarget="sidebar" className="flex flex-col gap-y-2" dataLength={data?.pages.length * 10} next={fetchNextPage} hasMore={hasNextPage ?? false} loader={<PlaylistSkeleton />}>
      {data?.pages.map((d: ISpotifyUserPlaylistResponse) =>
        d.items.map((p, i) => (
          <div onClick={() => router.push(`/playlist/${p.id}`)} key={i} className="cursor-pointer flex justify-center items-center gap-4 rounded-md bg-base-100">
            ...
            <a title={p.name.length > 15 ? p.name : ""} className="font-bold w-full overflow-hidden whitespace-nowrap">
